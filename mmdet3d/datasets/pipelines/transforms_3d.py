...
class RandomFlip3D(RandomFlip):
    ...
    def update_transform(self, input_dict):
        ...
        transform[:,:3,:3] = input_dict['img_inputs'][1] # rot
        transform[:,:3,-1] = input_dict['img_inputs'][2] # trans
        transform[:, -1, -1] = 1.0
    ...
    def __call__(self, input_dict):
        ...
        # no 'img_fields', 'bbox_fields', 'mask_fields', 'seg_fields' in input_dict.keys(), do nothing here
        # so no update_transform in RandomFlip3D is ok
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        # trans_factor = np.array(self.translation_std, dtype=np.float32)
...
class NormalizeMultiviewImage(object):
    ...
    def __init__(self, mean, std, to_rgb=True, norm_zero_one=True):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        if self.norm_zero_one:
            results["img"] = [img / 255 for img in results["img"]]
        results["img"] = [
