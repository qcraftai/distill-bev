...
device='cuda'
model = dict(
    type='BEVDepth4DDistill',
    # teacher_config='configs/centerpoint/centerpoint_02pillar_second_secfpn_circlenms_4x8_cyclic_20e_nus.py',
    # teacher_ckpt='output/centerpoint_02pillar_second_secfpn_circlenms_4x8_cyclic_20e_nus/20210816_064624-0f3299c0.pth',
    teacher_config='configs/centerpoint/centerpoint_02pillar_second_secfpn_dcn_4x8_cyclic_20e_nus.py',
    teacher_ckpt='output/centerpoint/centerpoint_02pillar_second_secfpn_dcn_4x8_cyclic_20e_nus/epoch_20.pth',
    self_ckpt=None,
    distill_type='fgd',
    distill_params=dict(
        ...
        spatial_t=0.5,
        spatial_student_ratio=1.0,
        channel_t=0.5,
        fg_feat_loss_weights=[1.5e-3,],
        bg_feat_loss_weights=[4e-2,],
        channel_loss_weights=[0.25,],
        spatial_loss_weights=[2.5e-3,],
        adaptation_type='1x1conv',
        student_adaptation_params=dict(kernel_size=1, stride=1, upsample_factor=4), # for TwoLayer adaptation in fgd
        teacher_adaptation_type='identity',
        teacher_adaptation_params=dict(kernel_size=4, stride=4), # for TwoLayer adaptation in fgd
        spatial_attentions=['teacher',],
        feat_criterion=dict(type='MSELoss', reduction='none'),
        spatial_criterion=dict(type='L1Loss', reduction='none'),
        channel_criterion=dict(type='L1Loss', reduction='none'),
        transpose_mask=False,
        foreground_mask='gt',
        background_mask='logical_not',
        scale_mask='combine_gt',
        spatial_mask=True,
        channel_mask=True,
        student_feat_pos=['head',],
        teacher_feat_pos=['head',],
        two_stage_epoch=-1,
        affinity_weights=[0, ],
        affinity_mode='none',
        affinity_criterion=dict(type='SmoothL1Loss'),
        affinity_split=1,
        non_empty_weight=0,
        output_threshold=1.0,
        groundtruth_threshold=None,
        fp_as_foreground='none',
        fp_weight=0,
        fp_epoch=0,
        multi_scale_epoch=-1,
        fp_scale_mode='dfs',
        gauss_fg_weight=-1e10,
        context_length=0,
        context_weight=0,
    ),
    aligned=True,
