...
def create_groundtruth_database(dataset_class_name,
    ...
    elif dataset_class_name == 'NuScenesDataset' and not virtual:
        dataset_cfg.update(
            use_valid_flag=True,
            pipeline=[
                dict(
                ...
                dict(
                    ...
                    sweeps_num=10,
                    interval=1,  # avoid random choosing 10 sweeps from max sweeps
                    use_dim=[0, 1, 2, 3, 4],
    ...
    elif dataset_class_name == 'NuScenesDataset' and virtual:
        dataset_cfg.update(
            use_valid_flag=True,
            pipeline=[
                dict(
    ...
    if database_save_path is None:
        database_save_path = osp.join(data_path, f'{info_prefix}_gt_database') \
            if not virtual else osp.join(data_path, f'{info_prefix}_virtual_gt_database')
    if db_info_save_path is None:
        db_info_save_path = osp.join(data_path, f'{info_prefix}_dbinfos_train.pkl') \
            if not virtual else osp.join(data_path, f'{info_prefix}_virtual_dbinfos_train.pkl')
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        for i in range(num_obj):
            ...
            rel_filepath = osp.join(f'{info_prefix}_gt_database', filename) if not virtual else \
                osp.join(f'{info_prefix}_virtual_gt_database', filename)
