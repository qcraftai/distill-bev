...
class CenterHead(BaseModule):
            ...
            Returns:
                tuple[list[torch.Tensor]]: Tuple of target including \
                    ...
                    - list[torch.Tensor]: Masks indicating which \
                        boxes are valid.
        """
        # gt_bboxes_3d is a list of LiDARInstance3DBoxes
        # gt_labels_3d is a list of torch.Tensor
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d,
                           custom_radius_func=None, custom_scale_func=None,
                           custom_rotate_gauss_func=None, custom_draw_gauss_func=None):
        ...
        for class_name in self.class_names:
            # torch.where returns the indices of non-zero elements
            task_masks.append([
        ...
        if custom_scale_func is not None:
            scale_mask = gt_bboxes_3d.new_zeros(
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    radius_func = custom_radius_func if custom_radius_func is not None else gaussian_radius
                    radius = radius_func((length, width), min_overlap=self.train_cfg['gaussian_overlap'])
                    ######################
                    ...
                    #################
                    # if custom_rotate_gauss_func is not None:
                    #     yaw = task_boxes[idx][k][6]
                    #     _, sigma = custom_rotate_gauss_func(torch.tensor([coor_x, coor_y, width, length, yaw], device=device))
                    # else:
                    #     sigma = None
                    # draw_func = draw_gaussian if custom_draw_gauss_func is None else custom_draw_gauss_func
                    # draw_func(heatmap[cls_id], center_int, radius, sigma=sigma)
                    draw_gaussian(heatmap[cls_id], center_int, radius)
                    ######################
                    if custom_scale_func is not None:
                        custom_scale_func(scale_mask, center_int, (length, width), radius)
                    ######################
        ...
        if custom_scale_func is not None:
            return heatmaps, anno_boxes, inds, masks, scale_mask
        else:
            return heatmaps, anno_boxes, inds, masks


    @force_fp32(apply_to=('preds_dicts'))
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, get_targets=False, **kwargs):
        ...
        loss_dict = dict()
        # when input to CenterPointHead is of size (4, 384, 128, 128)
        # preds_dicts is a list of 6 elements (6 tasks)
        # each element is a length 1 list
        # the only element of that length 1 list is a list of 6 elements
        # preds_dicts[0][0]['reg'] is of size (4, 2, 128, 128)
        # preds_dicts[0][0]['height'] is of size (4, 1, 128, 128)
        # preds_dicts[0][0]['dim'] is of size (4, 3, 128, 128)
        # preds_dicts[0][0]['rot'] is of size  (4, 2, 128, 128)
        # preds_dicts[0][0]['vel'] is of size (4, 2, 128, 128)
        # preds_dicts[0][0]['heatmap'] is of size (4, 1, 128, 128)
        ...
        if get_targets:
            return loss_dict, heatmaps, anno_boxes, inds, masks
        else:
            return loss_dict
