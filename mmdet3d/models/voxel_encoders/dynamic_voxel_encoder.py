...
def voxelization_virtual(points, pc_range, voxel_size):    
    ...
    real_points = points[real_points_mask][:, [0, 1, 2, 3, 4, -1]]
    painted_point = points[painted_points_mask]  
    virtual_point = points[virtual_points_mask] 

    padded_points = torch.zeros(len(points), 24, device=points.device, dtype=points.dtype)

    # real points will occupy channels 0 to 5 and -1 
    padded_points[:len(real_points), :6] = real_points
    padded_points[:len(real_points), -1] = 1 

    # painted points will occupy channels 6 to 23 
    padded_points[len(real_points):len(real_points)+len(painted_point), 6:21] = painted_point[:, :-2]
    padded_points[len(real_points):len(real_points)+len(painted_point), 21] = painted_point[:, -2]
    padded_points[len(real_points):len(real_points)+len(painted_point), 22] = 1
    padded_points[len(real_points):len(real_points)+len(painted_point), 23] = 0

    #  virtual points will occupy channels 6 to 23 
    padded_points[len(real_points)+len(painted_point):, 6:21] = virtual_point[:, :-2]
    padded_points[len(real_points)+len(painted_point):, 21] = virtual_point[:, -2]
    padded_points[len(real_points)+len(painted_point):, 22] = 0
    padded_points[len(real_points)+len(painted_point):, 23] = 0
    ...
    voxels[mix_mask, :6] = voxels[mix_mask, :6] / indicator[mix_mask].unsqueeze(-1)
    voxels[mix_mask, 6:] = voxels[mix_mask, 6:] / (1-indicator[mix_mask].unsqueeze(-1))
    return voxels, unique_coords
