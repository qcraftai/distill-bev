...
# TODO: sanity check of modified basicblock, bottleneckblock
from ..bricks import Bottleneck, BasicBlock
...
class ResNetForBEVDet(nn.Module):
    def __init__(self, numC_input, num_layer=[2,2,2], num_channels=None, stride=[2,2,2],
                 backbone_output_ids=None, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU', inplace=True),
        ...
        if block_type == 'BottleNeck':
            ...
            for i in range(len(num_layer)):
                ...
                layer.extend([Bottleneck(curr_numC, curr_numC//4,
                                         norm_cfg=norm_cfg, act_cfg=act_cfg) for _ in range(num_layer[i]-1)])
            ...
            for i in range(len(num_layer)):
                layer=[BasicBlock(curr_numC, num_channels[i], stride=stride[i],
                                  ...
                                  norm_cfg=norm_cfg, act_cfg=act_cfg)]
                curr_numC= num_channels[i]
                layer.extend([BasicBlock(curr_numC, curr_numC, norm_cfg=norm_cfg, act_cfg=act_cfg) for _ in range(num_layer[i]-1)])
                layers.append(nn.Sequential(*layer))
        else:
            assert False
    ...
    def forward(self, x):
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if lid in self.backbone_output_ids:
                feats.append(x_tmp)
        # (8, 64, 128, 128) -> [(8, 128, 64, 64), (8, 256, 32, 32), (8, 512, 16, 16)]
        return feats