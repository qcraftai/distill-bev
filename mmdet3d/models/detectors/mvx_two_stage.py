...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        self.test_dist2velo = test_dist2velo
        self.lidar_interval = lidar_interval # sync with lidar interval in loading.py
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False, gt_bboxes_3d=None, gt_time_interval=None):
        ...
        if pts_feats and self.with_pts_bbox:
            ...
            ################
            # right now only support lidar-based model
            # TODO: get time interval from load annotations and check consistency
            if self.test_dist2velo:
                assert isinstance(points, list) and len(points) == 1
                sorted_timestamps = torch.unique(points[0][:, -1], sorted=True)
                time_interval = torch.abs(sorted_timestamps[-1] - sorted_timestamps[0])
                if time_interval.item() == 0:
                    time_interval = torch.tensor(self.lidar_interval)
                # absolute speed here. don't use bevdet4d anno file! it uses ego-relative speed
                # assume lidar instance bbox format here (nuscenes)
                time_interval = time_interval.to(bbox_pts[0]['boxes_3d'].device)
                for pts_bbox in bbox_pts:
                    assert isinstance(pts_bbox['boxes_3d'] ,LiDARInstance3DBoxes)
                    bboxes_3d = pts_bbox['boxes_3d'].tensor
                    bboxes_3d[:, 7:9] = bboxes_3d[:, 7:9] / time_interval
                    pts_bbox['boxes_3d'] = LiDARInstance3DBoxes(bboxes_3d,
