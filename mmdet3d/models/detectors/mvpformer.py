...
class MVPFormer(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None):
        super(MVPFormer,
              self).__init__(None, pts_voxel_encoder,
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        voxel_features, coors, _ = self.pts_voxel_encoder(pts)
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        bev_embed = outs['bev_embed'].permute(0, 2, 1)
        B, C, HW = bev_embed.shape
        H = W = int(HW**0.5)
        
        t_attention_mask = torch.mean(torch.abs(bev_embed), [1], keepdim=True)
        t_attention_mask = t_attention_mask.view(B, -1)
        t_attention_mask = torch.softmax(t_attention_mask / 0.5, dim=1)
        t_attention_mask = t_attention_mask.view(H, W)
