...
class CenterPoint(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas, return_canvas=False, return_backbone_feature=False):
        ...
        outputs = []
        # when input is of size 4*(240752, 5)
        # voxels is of size (99607, 20, 5), num_points is of size (99607,), coors is of size (99607, 4)
        # voxel_features is of size (99607, 64)
        voxels, num_points, coors = self.voxelize(pts)
        voxel_features = self.pts_voxel_encoder(voxels, num_points, coors)
        batch_size = coors[-1, 0] + 1
        # x is of size (64, 512, 512) when voxel_features is of size (99607, 64)
        x = self.pts_middle_encoder(voxel_features, coors, batch_size)
        # TODO: return x to compute empty_mask
    ...
    def forward_pts_train(self,
        ...
        # note losses might be a list of losses + heatmaps
        # centerpoint head loss computation changes preds, by performing clip_sigmoid on it
        losses = self.pts_bbox_head.loss(gt_bboxes_3d, gt_labels_3d, outs, get_targets=get_targets)
        if get_preds:
            return outs, losses
        else:
            return losses
