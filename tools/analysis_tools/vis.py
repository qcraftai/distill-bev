...
def parse_args():
    ...
    parser.add_argument('--start-vis-frames',
                        type=int,
                        default=0,
                        help='index of the start frame for visualization')
    parser.add_argument('--end-vis-frames',
                        type=int,
                        default=500,
                        help='index of the end frame for visualization')
    ...
    parser.add_argument('--bev-draw-gt', action='store_true')
...
def main():
    ...
    for cnt, infos in enumerate(dataset['infos'][max(args.start_vis_frames, 0):\
            min(args.end_vis_frames, len(dataset['infos']))]):
        if cnt % 10 == 0:
            print('%d/%d' % (cnt, min(args.end_vis_frames - args.start_vis_frames, len(dataset['infos']))))
        # collect instances
        ...
        imgs = []
        for view in views:
            if '/mnt/vepfs/ML/ml-public/nuscenes_virtual/' in infos['cams'][view]['data_path']:
                infos['cams'][view]['data_path'] = infos['cams'][view]['data_path'].replace('/mnt/vepfs/ML/ml-public/nuscenes_virtual/', '/mnt/vepfs/ML/ml-public/Stuff/nuscenes_virtual/')
            ...
            for aid in range(valid.shape[0]):
                for index in draw_boxes_indexes_img_view:
                    ...
                    ###############
                        if scores[aid] < args.vis_thred and pred_flag[aid]:
                            continue
                        cv2.line(img,
            ...
            if args.format == 'image':
                cv2.imwrite(os.path.join(vis_dir, f'{id}_{view}.jpg'), img)
        ...
        if args.bev_draw_gt:
            ...
            pred_flag = np.concatenate([pred_flag, np.logical_not(gt_flag)],
                                       axis=0)
            scores = scores.tolist()+[0 for _ in range(infos['gt_boxes'].shape[0])]
            scores = np.array(scores, dtype=np.float32)
            sort_ids = np.argsort(scores)
        ...
        # white background
        # canvas = np.ones((int(canva_size), int(canva_size), 3), dtype=np.uint8) * 250
        # draw lidar points
        if '/mnt/vepfs/ML/ml-public/nuscenes_virtual/' in infos['lidar_path']:
            infos['lidar_path'] = infos['lidar_path'].replace('/mnt/vepfs/ML/ml-public/nuscenes_virtual/', '/mnt/vepfs/ML/ml-public/Stuff/nuscenes_virtual/')
        ...
        for p in lidar_points:
            if check_point_in_img(p.reshape(1,3),
                ...
                cv2.circle(canvas, (int(p[0]), int(p[1])),
                           ...
                           radius=0, color=(169, 169, 169), thickness=2) # grey point cloud
                           # radius=0, color=(0, 0, 0), thickness=3)  # grey point cloud
        ...
        id = infos['token']
        if args.format == 'image':
            cv2.imwrite(os.path.join(vis_dir, f'{id}_bev.jpg'), canvas)
        ...
        # new layout
        img = np.zeros((900 * 2, 1600 * 3 + canva_size * scale_factor, 3),
                       dtype=np.uint8)
        img[:900, :1600*3, :] = np.concatenate(imgs[:3], axis=1)
        img[900:, :1600*3, :] = np.concatenate(imgs[3:], axis=1)
        img = cv2.resize(img, (int(1600 / scale_factor * 3 + canva_size),
                               int(900 / scale_factor * 2)))
        h_begin = int((900 * 2 / scale_factor - canva_size) // 2)
        img[0:900, int(1600 / scale_factor * 3):, :] = canvas
        #########################
        if args.format == 'image':
            # TODO: save in cnt, not token
            # cv2.imwrite(os.path.join(vis_dir, '%s.jpg' % infos['token']), img)
            cv2.imwrite(os.path.join(vis_dir, '%s.jpg' % (cnt+args.start_vis_frames)), img)
